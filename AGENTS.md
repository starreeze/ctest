...
- Adaptive HTTPS measurement follows at most one HTTPS redirect, sends `Referer` from the test URL origin, reads exactly the requested number of body bytes, and treats a failed larger ramp size as a fallback to the last successful size with `--speed_http_ramp_fail_factor`. Probe wall time defaults to the bounded `--speed_http_max_transfer_seconds`; a positive `--speed_http_deadline_rate_kibps` tightens it to `--speed_http_connect_overhead` plus size / rate, capped by that maximum. `--speed_test_retry` is sdk-only; adaptive retries are `--speed_http_trials` (default 2).
