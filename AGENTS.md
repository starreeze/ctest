...
3. `function/speed.py` latency-tests every candidate, groups survivors by host, and speed-tests each host's endpoints in latency order until one transfers data. It records one host outcome in SQLite, renames the winning endpoint as `latency - speed - original_name`, and reconstructs proxy groups semantically.
...
- Candidate deduplication is keyed by normalized host:port, while failure persistence is keyed by host. Test all eligible host:port candidates, then retain the first latency-ranked endpoint per host with positive throughput.
...
- Adaptive HTTPS measurement follows at most one HTTPS redirect, sends `Referer` from the test URL origin, reads exactly the requested number of body bytes, and treats a failed larger ramp size as a fallback to the last successful size with `--speed_http_ramp_fail_factor`. Probe wall time defaults to the bounded `--speed_http_max_transfer_seconds`; a positive `--min_speed_threshold_kbps` tightens it to `--speed_http_connect_overhead` plus size / threshold, capped by that maximum. `--speed_test_retry` is sdk-only; adaptive retries are `--speed_http_trials` (default 2). For each host, stop throughput testing after the first latency-ranked endpoint produces positive traffic.
- A host succeeds when one latency-valid endpoint produces positive throughput; failures advance a host cooldown through 0/23/71/167 hours, capped at 167 hours. Suppress speed-derived failures when their ratio reaches `--speed_outage_fail_ratio` after `--speed_outage_min_samples` hosts.
