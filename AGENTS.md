...
- `common/db.py`: persistent host-level failure tracking with 1/3/7-day retry cooldowns.
...
- A host succeeds when one latency-valid endpoint produces positive throughput; failures advance a host cooldown by 1/3/7 days. Suppress speed-derived failures when their ratio reaches `--speed_outage_fail_ratio` after `--speed_outage_min_samples` hosts.
