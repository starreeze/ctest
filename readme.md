...
It consists of the following [free link collections](urls.txt) (GitHub lines already rewritten to Fastly jsDelivr) and my custom config file. Date-token feeds such as mibei are expanded at `update()` time and are omitted from this static example. You can quote the urls and combine them manually to form the above subscription link.
