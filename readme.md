...
The full pipeline runs update, fix, latency, and throughput processing against a sibling staging copy. The selected profile stays untouched until every stage and the temporary Mihomo process have completed successfully, then the staging file atomically replaces it. If any step fails, the staging file is removed and the selected profile never changes. The standalone `fix` and `speed` commands use the same staging-and-replace behavior.
